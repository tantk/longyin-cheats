...
function MT.hook.discover(fastMode)
  ...
  if not S.base or S.base == 0 then log("失败: GameAssembly.dll未加载 GA not loaded"); return false, "GameAssembly.dll not loaded" end
  if not MT.hook.allocWorkArea() then log("失败: 分配工作区失败 alloc work area failed"); return false, "Work area alloc failed" end

  local classFromName = getAddress("GameAssembly.il2cpp_class_from_name")
  local objNew = getAddress("GameAssembly.il2cpp_object_new")
  if not objNew or objNew == 0 then log("失败: il2cpp导出函数缺失 il2cpp_object_new missing"); return false, "il2cpp exports missing" end
  S.objNew = objNew
  ...
  if not gcClass or gcClass == 0 then log("失败: GameController类未找到 GC class not found"); return false, "GameController not found" end
  ...
  if not gc or gc == 0 then log("失败: GC实例为空——请先加载存档 GC._instance null"); return false, "GC._instance null (load save)" end
  S.gc = gc
  log("gc=" .. toHex(gc))
  ...
  if not wd or wd == 0 then log("失败: 世界数据为空 WorldData null"); return false, "WorldData null" end
  local hl = readQword(wd + 0x50)
  if not hl or hl == 0 then log("失败: 角色列表为空 HerosList null"); return false, "HerosList null" end
  ...
  if not ph or ph == 0 then log("失败: 主角未找到 Hero not found"); return false, "Hero not found" end
  S.hero = ph
  ...
  if not S.itemKlass then log("失败: 背包中无物品 No items in inventory"); return false, "No items in inventory" end
