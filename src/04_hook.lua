...
function MT.hook.discover(fastMode)
  ...
  if not S.base or S.base == 0 then log("FAIL: GA not loaded"); return false, "GameAssembly.dll not loaded" end
  if not MT.hook.allocWorkArea() then log("FAIL: alloc work area"); return false, "Work area alloc failed" end

  local classFromName = getAddress("GameAssembly.il2cpp_class_from_name")
  local objNew = getAddress("GameAssembly.il2cpp_object_new")
  if not objNew or objNew == 0 then log("FAIL: il2cpp_object_new missing"); return false, "il2cpp exports missing" end
  S.objNew = objNew
  ...
  if not gcClass or gcClass == 0 then log("FAIL: GC class not found"); return false, "GameController not found" end
  ...
  if not gc or gc == 0 then log("FAIL: GC._instance null — need to load a save first"); return false, "GC._instance null (load save)" end
  S.gc = gc
  log("gc=" .. toHex(gc))
  ...
  if not wd or wd == 0 then log("FAIL: WorldData null"); return false, "WorldData null" end
  local hl = readQword(wd + 0x50)
  if not hl or hl == 0 then log("FAIL: HerosList null"); return false, "HerosList null" end
  local ip = readQword(hl + 0x10)
  if not ip or ip == 0 then log("FAIL: HerosList items null"); return false, "HerosList items null" end
  local ph = readQword(ip + 0x20)
  if not ph or ph == 0 then log("FAIL: Hero not found"); return false, "Hero not found" end
  S.hero = ph
  ...
  if not ail or ail == 0 then log("FAIL: allItem list null"); return false, "allItem list null" end
  ...
  if not S.itemKlass then log("FAIL: No items in inventory (empty?)"); return false, "No items in inventory" end
  log("Data OK: wd=" .. toHex(wd) .. " hero=" .. toHex(ph) .. " items=" .. ic)
  ...
  log("Resolved " .. rvaCount .. " RVAs")

  -- Install main-thread hook
  log("Installing main-thread hook...")
  local hookOk, hookErr = MT.hook.installMainThreadHook()
  if not hookOk then log("FAIL: hook install: " .. tostring(hookErr)); return false, "Hook: " .. (hookErr or "unknown") end
  log("Hook installed OK")
